...
**How I found the root cause:** `POST /songs/<id>/rate` in [routes/songs.py:29](routes/songs.py#L29) calls `rate_song()` in [services/notification_service.py:73](services/notification_service.py#L73). I read `rate_song()` alongside its sibling `add_to_playlist()` in the same file. `add_to_playlist()` ends with a `create_notification()` call guarded by `song.shared_by != added_by_user_id`. `rate_song()` saves the rating, commits, but it does not return with a `create_notification()` call at all. The `create_notification` docstring even lists `'song_rated'` as an intended type, confirming the notification was meant to exist and was simply never wired up.
